...
export class OllamaService {
  ...
  async groundedAnswer(context: string, question: string): Promise<string> {
    const response = await fetch(`${this.url}/api/generate`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ model: this.model, prompt: `Use only this context and cite its source IDs.\\n${context}\\nQuestion: ${question}`, stream: false }) });
